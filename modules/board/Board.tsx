...
function AddSticker({ columnId }: { columnId: StickerColumnId }) {
  ...
  return (
    <form
      onSubmit={e => { e.preventDefault(); addSticker(columnId, title); setTitle(''); }}
    ...
    >
      <input
        ...
        className="min-w-0 flex-1 border border-black/15 bg-white/80 px-2 py-2 text-xs outline-none focus:border-black/40"
      />
        ...
        {cols.map(col => {
          ...
          return (
            <div key={col.id} className="min-w-[258px] max-w-[258px]">
              <div
                className="mb-2 flex items-center justify-between px-3 py-[11px]"
                style={{ background: col.header, color: col.headerText, borderRadius: '9px 9px 4px 4px', boxShadow: '0 2px 0 rgba(0,0,0,.08)' }}
              >
              ...
              <div className="mt-2 flex flex-col gap-2">
                {colStickers.map(st => (
                  <div
                    key={st.id}
                    className="relative cursor-pointer rounded-lg p-3 text-black transition-transform hover:scale-[1.012]"
                    style={{
                  ...
                  >
                    {st.selected && (
                      <span className="absolute -top-2 right-2 rounded-full bg-[var(--ink)] px-2 py-[2px] font-mono text-[7px] uppercase tracking-[0.08em] text-white">
                        EN ENSAMBLAJE
                      </span>
                    )}
                    <textarea
                  ...
                  <div className="mt-2 flex flex-wrap gap-1">
                    {taskCols.map(dest => (
                      <button
                        ...
                        className="border border-black/15 px-2 py-1 font-mono text-[9px] uppercase"
                      >
                        {dest.title}
                      </button>
                    ))}
