...
export function Hoy() {
  const { state, addTask, updateTask, setState } = useLanka();
    ...
    <div>
      <div className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <Card>
          ...
          <p className="mt-2 text-4xl font-black">{todayTasks.length}</p>
        </Card>
        <Card>
          <p className="font-mono text-[10px] uppercase text-[var(--muted)]">Vencidas</p>
          <p className={`mt-2 text-4xl font-black ${overdue.length > 0 ? 'text-[var(--terra)]' : ''}`}>{overdue.length}</p>
        </Card>
        <Card>
          <p className="font-mono text-[10px] uppercase text-[var(--muted)]">KPIs bajo meta</p>
          <p className={`mt-2 text-4xl font-black ${criticalKpis.length > 0 ? 'text-[var(--terra)]' : ''}`}>{criticalKpis.length}</p>
        </Card>
      </div>
      ...
      <Card className="mb-4">
        ...
        {todayTasks.length === 0 && overdue.length === 0 ? (
          <p className="border-l-4 border-[var(--acid)] bg-[rgba(200,216,75,.18)] p-3 text-sm">
          ...
          <div className="space-y-2">
            {[...overdue, ...todayTasks].map(t => (
              <div
                key={t.id}
                className={`flex items-start gap-3 bg-[var(--bone)] p-3 ${t.dueAt && t.dueAt < today ? 'border-l-4 border-l-[var(--terra)]' : 'border-l-4 border-l-[var(--green)]'}`}
              >
                <button
                  onClick={() => updateTask(t.id, { done: true, status: 'done' })}
                  className="mt-0.5 h-4 w-4 flex-shrink-0 border-2 border-[var(--ink)]"
                />
      ...
      {criticalKpis.length > 0 && (
        <Card className="mb-4">
              ...
              return (
                <div key={kpi.id} className="flex items-center justify-between gap-3 border border-[var(--line)] p-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="font-mono text-[10px] text-[var(--muted)]">{kpi.current} / {kpi.target} {kpi.unit} · {ratio}%</p>
                  ...
                  <button
                    ...
                    className="flex-shrink-0 border border-[var(--terra)] px-3 py-2 font-mono text-[10px] font-bold uppercase text-[var(--terra)]"
                  >
                    + Tarea
                  </button>
                </div>
          ...
          <p className="mt-3 font-mono text-[10px] text-[var(--muted)]">
            Para actualizar métricas → <span className="font-bold">Sistema › Métricas</span>
          </p>
        </Card>
      )}
        ...
        <Card>
          ...
          <div className="space-y-1">
            {state.activity.slice(0, 8).map((item, i) => (
              <p key={i} className="border-b border-black/10 pb-2 font-mono text-[11px] text-[var(--muted)]">{item}</p>
            ))}
          </div>
        </Card>
      )}
