...
import type { AssemblyKind, ClientCase, EditorialEntry, LankaState, ModeloSection, Owner, Priority, RoadmapItem, StickerColumnId, Task, TaskStatus, VaultItem, WorkspaceConfig } from './types';
...
async function loadFromDB(): Promise<LankaState | null> {
  try {
    ...
    const editorial = Array.isArray(ws.editorial) ? (ws.editorial as EditorialEntry[]) : [];
...
type Store = {
  ...
  addEditorial: (patch: Omit<EditorialEntry, 'id' | 'createdAt' | 'updatedAt'>) => void;
  updateEditorial: (id: string, patch: Partial<EditorialEntry>) => void;
...
export function LankaProvider({ children }: { children: React.ReactNode }) {
  ...
  }, [state.strategy, state.kpis, state.config, state.modelo, state.roadmap, state.editorial, loaded]);
  ...
  const store = useMemo<Store>(() => ({
    ...
    addEditorial(patch) {
      const e: EditorialEntry = {
        ...
        body: patch.body ?? '',
        source: patch.source ?? '',
        status: patch.status ?? 'borrador',
        relatedSeries: patch.relatedSeries ?? '',
        relatedChapter: patch.relatedChapter ?? '',
        relatedCase: patch.relatedCase ?? '',
        createdAt: now(),
        updatedAt: now(),
      };
      setState(s => ({ ...s, editorial: [e, ...(s.editorial ?? [])] }));
    },

    updateEditorial(id, patch) {
      setState(s => ({
        ...s,
        editorial: (s.editorial ?? []).map(e => e.id === id ? { ...e, ...patch, updatedAt: now() } : e),
      }));
    },

    deleteEditorial(id) {
      setState(s => ({ ...s, editorial: (s.editorial ?? []).filter(e => e.id !== id) }));
    },
